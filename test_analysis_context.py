...
monitor.requirements = [
    TaskRequirement(
        id="1", description="Create login endpoint", priority=1, completed=True
    ),
    TaskRequirement(
        id="2", description="Add JWT token generation", priority=1, completed=True
    ),
    TaskRequirement(
        id="3", description="Implement password hashing", priority=2, completed=False
    ),
    TaskRequirement(
        id="4", description="Add rate limiting", priority=3, completed=False
    ),
]

# Set some test statistics
monitor.stats = {"commands": 15, "deviations": 2}

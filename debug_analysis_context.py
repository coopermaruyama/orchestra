...
def main() -> None:
    ...
    parser.add_argument("--task-file", default=".claude-task.json",
    ...
    if task_file.exists():
        ...
        if config:
            ...
            monitor.requirements = [TaskRequirement(**req) for req in config.get('requirements', [])]
        ...
        monitor.requirements = [
            TaskRequirement(id="1", description="First requirement", priority=1, completed=True),
            TaskRequirement(id="2", description="Second requirement", priority=2, completed=False),
            TaskRequirement(id="3", description="Third requirement", priority=3, completed=False)
        ]

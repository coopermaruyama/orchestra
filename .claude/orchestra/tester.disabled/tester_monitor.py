...
class TesterMonitor(GitAwareExtension):
    def __init__(self, config_path: Optional[str] = None) -> None:
        ...
        self.logger = setup_logger(
            "tester_monitor", log_file, logging.DEBUG, truncate=True, max_length=300
        )
        self.logger.info("TesterMonitor initialized")

        # Initialize base class
        base_working_dir = working_dir or "."
        super().__init__(config_file=config_path, working_dir=base_working_dir)
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        elif "calibration" in state and state["calibration"] is not None:
            ...
            for todo in todos:
                if todo.get("status") == "completed":
                    ...
                    for old_todo in self.synced_todos:
                        if (
                            old_todo.get("id") == todo_id
                            and old_todo.get("status") == "completed"
                        ):
                            was_completed = True
                            break
    ...
    def calibrate(self) -> None:
        ...
        print(
            "I'll use the test-calibrator subagent to have a conversation with you.\n"
        )
        ...
        if hasattr(self, "invoke_subagent"):
            ...
            if memory_calibration:
                ...
                print(
                    "⚠️  Calibration incomplete. The calibrator should save to .claude/orchestra/tester/calibration.json"
                )
        else:
            # Fallback for testing
            self.calibration = TestCalibration(calibrated_at=datetime.now().isoformat())
            self.save_config()
    ...
    def run_tests(self, task_id: Optional[str] = None) -> None:
        ...
        if hasattr(self, "invoke_subagent"):
            ...
            if results_dir.exists():
                result_files = sorted(
                    results_dir.glob("*.json"), key=lambda x: x.stat().st_mtime
                )
                if result_files:
    ...
    def show_status(self) -> None:
        ...
        if self.test_results:
            ...
            for result in self.test_results[-5:]:  # Show last 5
                ...
                print(
                    f"   {status} {result.task_description[:50]}... ({result.timestamp})"
                )
        else:
            print("\n📊 No test results yet")

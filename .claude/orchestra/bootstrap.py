...
Ensures Orchestra is installed before running commands
"""
...
def check_orchestra_installed():
    """Check if orchestra command is available in PATH"""
    try:
...
def show_install_instructions():
    """Show one-time installation instructions"""
    flag_file = Path.home() / '.claude' / '.orchestra-install-shown'
    
    # Check if we've already shown instructions this session
    if flag_file.exists():
        return

...
class Orchestra:
    ...
    def enable(self, extension: str, scope: str = "global") -> None:
        ...
        bootstrap_path = "$HOME/.claude/orchestra/bootstrap.sh" if scope == "global" else ".claude/orchestra/bootstrap.sh"
    ...
    def _copy_extension_files(self, extension: str, scripts_dir: Path) -> None:
        ...
        monitor_source = Path(__file__).parent / "extensions" / extension / monitor_script
        ...
        if monitor_source.exists():
            ...
            self.console.print(f"[bold red]⚠️ Warning:[/bold red] {monitor_script} not found at {monitor_source}")
            return
        ...
        for module in extra_modules:
            ...
            if module_source.exists():
                ...
                self.console.print(f"[bold red]⚠️ Warning:[/bold red] {module} not found for {extension} extension")
    ...
    def _create_extension_commands(self, extension: str, commands_dir: Path, bootstrap_path: str) -> None:
    ...
    def _create_timemachine_commands(self, commands_dir: Path, bootstrap_path: str) -> None:
    ...
    def _create_hooks_config(self, commands_dir: Path, bootstrap_path: str, extension: str) -> None:
        """Create hooks configuration for Claude Code"""
        context = {
            "hook_command": f"sh {bootstrap_path} hook"
        }
        ...
        if settings_file.exists():
            try:
                ...
                self.console.print(f"[yellow]⚠️ Invalid JSON in settings.json, created backup at {backup_file}[/yellow]")
                existing_settings = {
        ...
        if source_agents_dir.exists():
            ...
            if agent_count > 0:
                if extension == "task":
                    ...
                    self.console.print(f"[bold green]🤖 Installed {agent_count} subagents[/bold green] for test automation")

    def _print_success_message(self, extension: str, scope: str, commands_dir: Path, scripts_dir: Path) -> None:
        ...
        self.console.print(f"[bold]🪝 Hooks:[/bold] Configured in {commands_dir.parent / 'settings.json'}")
    ...
    def _clean_hooks_from_settings(self, commands_dir: Path, bootstrap_path: Path) -> None:
        """Clean up hooks from settings.json"""
        settings_file = commands_dir.parent / "settings.json"
        ...
        try:
            ...
            if "hooks" in settings:
                ...
                for event_name in ["PreToolUse", "PostToolUse", "UserPromptSubmit", "Stop", "SubagentStop", "PreCompact"]:

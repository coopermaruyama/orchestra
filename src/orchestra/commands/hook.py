...
def invoke_monitor_hook(monitor: Dict[str, Any], hook_type: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout:
            ...
            error_msg = result.stderr or f"Monitor {monitor['extension']} failed with code {result.returncode}"
            return {
                "error": error_msg,
                "continue": True
            }
        ...
        return {
            "error": f"Monitor {monitor['extension']} timed out",
            "continue": True
        }

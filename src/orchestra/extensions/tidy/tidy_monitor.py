...
try:
    ...
    from .tool_runners import ParallelToolRunner, ToolResult, ToolRunnerFactory
except ImportError:
...
class TidyMonitor(BaseExtension):
    ...
    def _format_check_results(
        self, results: Dict[str, ToolResult], is_subagent: bool = False
    ) -> Dict[str, Any]:
        ...
        if failed_tools:
            ...
            for tool_name in failed_tools:
                ...
                if result.issues_count > max_issues:
                    output_lines.append(
                        f"  ... and {result.issues_count - max_issues} more issues"
                    )
        ...
        fixable_tools = [
            name for name, r in results.items() if r.can_fix and not r.success
        ]
        if fixable_tools and self.settings.get("auto_fix", False):
            output_lines.append("  • Auto-fix is enabled. Run: orchestra tidy fix")
        elif fixable_tools:
            output_lines.append(f"  • {len(fixable_tools)} tool(s) can auto-fix issues")
            output_lines.append("  • Run: orchestra tidy fix")
        ...
        if is_subagent:
            ...
            return {"continue": True, "suppressOutput": False, "output": output}
    ...
    def _cmd_init(self) -> str:
        ...
        if self.project_info:
            ...
            self.console.print(
                f"📦 Package Manager: [cyan]{self.project_info['package_manager']}[/cyan]"
            )
        else:
        ...
        self.settings["auto_fix"] = Confirm.ask(
            "Enable auto-fix for fixable issues?", default=False
        )
        self.settings["strict_mode"] = Confirm.ask(
            "Enable strict mode (fail on any issue)?", default=True
        )
        self.settings["parallel_execution"] = Confirm.ask(
            "Run tools in parallel?", default=True
        )
        self.settings["use_sidecar"] = Confirm.ask(
            "Enable sidecar mode (async background fixes)?", default=False
        )
        ...
        if Confirm.ask(
            ...
            while True:
                ...
                fix_command = Prompt.ask(
                    "Fix command (optional, press Enter to skip)", default=""
                )

                custom_command: Dict[str, str] = {"name": name, "command": command}
                if fix_command:
        ...
        for tool_name, result in results.items():
            if result.issues:
                ...
                if len(result.issues) > 5:
                    self.console.print(
                        f"  ... and {len(result.issues) - 5} more issues"
                    )
    ...
    def _cmd_fix(self, args: str) -> str:
        ...
        # Display results
        total_fixed = sum(
            r.fixed_count for r in results.values() if hasattr(r, "fixed_count")
        )
    ...
    def _cmd_status(self) -> str:
        ...
        # Project info
        panel_content.append("[bold]Project Information:[/bold]")
        if self.project_info:
            panel_content.append(f"  Type: {self.project_info.get('type', 'Unknown')}")
            panel_content.append(
                f"  Package Manager: {self.project_info.get('package_manager', 'Unknown')}"
            )
        else:
            panel_content.append("  Type: Unknown")
            panel_content.append("  Package Manager: Unknown")
        ...
        panel_content.append("\n[bold]Settings:[/bold]")
        panel_content.append(
            f"  Auto-fix: {'Yes' if self.settings.get('auto_fix', False) else 'No'}"
        )
        panel_content.append(
            f"  Strict Mode: {'Yes' if self.settings.get('strict_mode', True) else 'No'}"
        )
        panel_content.append(
            f"  Parallel Execution: {'Yes' if self.settings.get('parallel_execution', True) else 'No'}"
        )
        panel_content.append(
            f"  Sidecar Mode: {'Yes' if self.settings.get('use_sidecar', False) else 'No'}"
        )
    ...
    def _cmd_learn(self, args: str) -> str:
        ...
        if learn_type.lower() in ["dont", "don't"]:
    ...
    def _cmd_sidecar(self, args: str) -> str:
        ...
        if action == "merge":
            ...
            if files_updated > 0:
                self.console.print(
                    f"[green]✅ Merged {files_updated} fixed files[/green]"
                )
            else:

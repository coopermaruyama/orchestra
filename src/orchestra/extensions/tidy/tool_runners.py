...
class ToolRunner(ABC):
    ...
    def __init__(self, working_dir: str = ".", command: str = "", fix_command: Optional[str] = None):
    ...
    @abstractmethod
    def parse_output(self, output: str, error: str, exit_code: int) -> List[Dict[str, Any]]:
            ...
            for file_result in data:
                for message in file_result.get("messages", []):
                    issues.append({
                        ...
                        "severity": "error" if message.get("severity", 0) == 2 else "warning",
...
class PrettierRunner(ToolRunner):
    ...
    def is_available(self) -> bool:
        output, error, exit_code = self._run_command(["prettier", "--version"], timeout=5)
        return exit_code == 0
    ...
    def parse_output(self, output: str, error: str, exit_code: int) -> List[Dict[str, Any]]:
        ...
        for line in error.split("\n") + output.split("\n"):
            if line.strip() and not line.startswith("Checking") and "needs formatting" in line:
...
class CustomCommandRunner(ToolRunner):
    ...
    def __init__(self, working_dir: str = ".", command: str = "", fix_command: Optional[str] = None, name: str = "custom"):
    ...
    def is_available(self) -> bool:
        ...
        if base_cmd in ["npm", "yarn", "pnpm", "make"]:
            output, error, exit_code = self._run_command([base_cmd, "--version"], timeout=5)
            return exit_code == 0
...
class ToolRunnerFactory:
    ...
    @staticmethod
    def create(tool_name: str, working_dir: str = ".", command: str = "", fix_command: Optional[str] = None) -> ToolRunner:
...
class ParallelToolRunner:
    ...
    def fix_all(self, files: Optional[List[str]] = None) -> Dict[str, ToolResult]:
        ...
        fixable_runners = [(name, runner) for name, runner in self.runners if runner.fix_command]

...
2. For each correction found, extract:
   - What the assistant did wrong
   - What the user said should have been done instead
   - The underlying principle or rule to follow

3. Format each finding as a clear, actionable instruction that could prevent the same mistake in the future.

4. If you find any corrections, respond with ONLY the formatted instructions, one per line, starting with "- ". 

5. If no corrections are found, respond with "NO_CORRECTIONS_FOUND".

Focus on actionable learnings that would help prevent similar mistakes in future conversations."""
    ...
    def _update_memory_file(self, corrections: str) -> None:
        ...
        try:
            ...
            if not existing_content.strip():
                existing_content = "# Never Again - Learned Corrections\n\nInstructions learned from user corrections to prevent repeated mistakes.\n\n"

            # Add timestamp and new corrections

...
""",
        ...
        assert any(
            "divide" in name.lower() or "zero" in name.lower() for name in test_names
        )
...
""",
            ...
            "calibration_data": {
                "test_commands": {"unit": "npm test", "watch": "npm test -- --watch"},
        ...
        assert any(
            "toggle" in reason.lower() or "complete" in reason.lower()
            for reason in test_reasons
        )
        assert any(
            "delete" in reason.lower() or "remove" in reason.lower()
            for reason in test_reasons
        )

        # Should follow Jest conventions
        assert all(
            test["file"].endswith(".test.js") or test["file"].endswith(".spec.js")
...
""",
        ...
        test_names = [test["test_name"] for test in result["tests_needed"]]
        assert any(
            "duplicate" in name.lower() or "existing" in name.lower()
            for name in test_names
        )
        assert any(
            "not found" in name.lower() or "404" in name.lower() for name in test_names
        )
...
""",
        ...
        assert any(
            keyword in test_reasons.lower()
            for keyword in ["boundary", "edge", "threshold", "tier", "zero", "empty"]
        )

        # Should test discount logic
        assert any(
            "discount" in test["test_name"].lower() for test in result["tests_needed"]
        )
...
""",
        ...
        assert any(
            keyword in test_names.lower()
            for keyword in ["invalid", "valid", "edge", "format", "boundary"]
        )

        # Should test multiple countries for phone
        assert any(
            "country" in test.get("reason", "").lower()
            or "UK" in test.get("reason", "")
            or "US" in test.get("reason", "")
            for test in result["tests_needed"]
        )

        # Should identify security testing needs
        assert any(
            "sanitize" in gap.lower() or "security" in gap.lower()
...
""",
        ...
        # Should test error cases
        assert any(
            "error" in test["reason"].lower() or "zero" in test["reason"].lower()
            for test in result["tests_needed"]
        )

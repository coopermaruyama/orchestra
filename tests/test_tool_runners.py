...
from orchestra.extensions.tidy.tool_runners import (
    ToolRunner, ToolResult, RuffRunner, BlackRunner, MypyRunner,
    ESLintRunner, PrettierRunner, CustomCommandRunner, 
    ToolRunnerFactory, ParallelToolRunner
)
...
class TestToolRunners(unittest.TestCase):
    ...
    def test_ruff_runner_check(self, mock_run):
        ...
        mock_run.return_value = MagicMock(
            stdout='[]',  # Empty JSON array means no issues
            stderr='',
            returncode=0
        )
    ...
    def test_parallel_tool_runner(self, mock_run):
        ...
        def side_effect(*args, **kwargs):
            ...
            if cmd == "ruff":
                ...
                return MagicMock(stdout='', stderr='would reformat test.py\n', returncode=1)
            else:
                return MagicMock(stdout='', stderr='', returncode=0)

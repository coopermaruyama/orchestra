...
class DataProcessor:
    ...
    def process(self, data: Optional[List[Dict]]) -> List[Dict]:
        ...
        assert any("import" in change["issue"].lower() for change in result["changes_made"])
        assert any("type" in change["issue"].lower() for change in result["changes_made"])
    
    @patch('orchestra.common.claude_cli_wrapper.ClaudeCLIWrapper.invoke')
    def test_fixes_complex_javascript_code(self, mock_invoke, command, messy_javascript_code):
...
def calculate_risk(value):
    ...
    return value * 0.1  # Placeholder calculation"""
        ...
        assert any("domain knowledge" in issue["reason"].lower() 
...
def authenticate(username, password):
    ...
    return {"token": "dummy"}'''
        ...
        mock_response = Mock(spec=ClaudeResponse)
        mock_response.success = True
        mock_response.content = original.replace("if not username or not password:", 
                                               "if not username or not password:")
        mock_invoke.return_value = mock_response

...
"""
    ...
    def test_full_workflow_hook_integration(self):
        ...
        stop_context = {"hook_event_name": "Stop", "stop_hook_active": False}
    ...
    def test_auto_detect_and_run_tools(self, mock_run):
        ...
        def mock_tool_check(cmd, *args, **kwargs):
            ...
            if cmd[0] == "black" and "--version" in cmd:
                return MagicMock(stdout="black, 23.0.0", stderr="", returncode=0)
            if cmd[0] == "mypy" and "--version" in cmd:
    ...
    def test_slash_command_workflow(self):
        ...
        # Test init command
        with patch(
            "orchestra.extensions.tidy.tidy_monitor.Confirm.ask"
        ) as mock_confirm:
            with patch(
                "orchestra.extensions.tidy.tidy_monitor.TidyMonitor._run_checks"
            ) as mock_checks:
                mock_confirm.side_effect = [
                    ...
                    True,
                    False,
                ]  # auto_fix, strict, parallel, custom
        ...
        # Test learn command
        result = monitor.handle_slash_command(
            "learn", "do Use descriptive variable names"
        )
        self.assertIn("Use descriptive variable names", monitor.do_examples)

        result = monitor.handle_slash_command(
            "learn", "don't Use single letter variables"
        )
    ...
    def test_parallel_tool_execution(self):
        ...
        with patch("orchestra.extensions.tidy.tool_runners.subprocess.run") as mock_run:
            ...
            def side_effect(cmd, *args, **kwargs):
                ...
                if "black" in cmd:
                    return MagicMock(
                        stdout="", stderr="would reformat src/main.py", returncode=1
                    )
                if "mypy" in cmd:
                    return MagicMock(
                        stdout="src/main.py:5: error: Missing type annotation",
                        stderr="",
                        returncode=1,
                    )
                return MagicMock(stdout="", stderr="", returncode=0)
    ...
    def test_subagent_stop_hook(self):
        ...
        with patch("orchestra.extensions.tidy.tool_runners.subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                stdout="[]", stderr="", returncode=0  # No issues
            )
    ...
    def test_pre_compact_hook(self):
        ...
        # Handle PreCompact
        context = {"hook_event_name": "PreCompact", "trigger": "manual"}

...
# Install with pip (user install by default)
if $PYTHON_CMD -m pip install --user . >/dev/null 2>&1; then
    echo "${GREEN}✅ Orchestra installed successfully!${NC}"
else
    echo "${YELLOW}⚠️  User install failed, trying with sudo...${NC}"
    if sudo $PYTHON_CMD -m pip install . >/dev/null 2>&1; then
        echo "${GREEN}✅ Orchestra installed successfully with sudo!${NC}"
    else
